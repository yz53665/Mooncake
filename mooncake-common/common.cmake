...
option(USE_NDS "option for using NDS (NPU Data Service) with NVMe over Fabric" OFF)
...
if (USE_NDS)
  if (NOT USE_NVMEOF)
    message(WARNING "USE_NDS requires USE_NVMEOF, enabling USE_NVMEOF")
    set(USE_NVMEOF ON)
    add_compile_definitions(USE_NVMEOF)
  endif()
  add_compile_definitions(USE_NDS)
  message(STATUS "NDS (NPU Data Service) support is enabled")

  # Set up Ascend toolchain paths for NDS (similar to USE_ASCEND)
  ...
  if(DEFINED ENV{ASCEND_HOME_PATH})
    message(STATUS "Use env ASCEND_HOME_PATH for NDS")
    file(GLOB ASCEND_TOOLKIT_ROOT "$ENV{ASCEND_HOME_PATH}/${CPU_ARCH}-linux")
    # Try cann subdirectory if ascend-toolkit pattern fails
    if(NOT ASCEND_TOOLKIT_ROOT OR NOT EXISTS "${ASCEND_TOOLKIT_ROOT}/include")
      file(GLOB ASCEND_TOOLKIT_ROOT "$ENV{ASCEND_HOME_PATH}/cann*/${CPU_ARCH}-linux")
    endif()
  else()
    # Try ascend-toolkit pattern first
    file(GLOB ASCEND_TOOLKIT_ROOT "/usr/local/Ascend/ascend-toolkit/latest/${CPU_ARCH}-linux")
    # Fall back to cann pattern if ascend-toolkit not found
    if(NOT ASCEND_TOOLKIT_ROOT OR NOT EXISTS "${ASCEND_TOOLKIT_ROOT}/include")
      file(GLOB ASCEND_TOOLKIT_ROOT "/usr/local/Ascend/cann*/${CPU_ARCH}-linux")
    endif()
  endif()
  ...
  if(EXISTS "${ASCEND_INCLUDE_DIR}/acl/acl.h")
    message(STATUS "Found Ascend include: ${ASCEND_INCLUDE_DIR}")
    message(STATUS "Found Ascend lib: ${ASCEND_LIB_DIR}")
  else()
    message(WARNING "Could not find Ascend headers. Ensure ASCEND_HOME_PATH is set correctly or Ascend/CANN is installed.")
  endif()
