...
tl::expected<size_t, ErrorCode> ThreeFSFile::write(std::span<const char> data,
    ...
    while (total_bytes_written < length) {
        ...
        if (gpu_staging::IsDevicePointer(
            ...
            auto ret = hf3fs_prep_npu_direct_io(
                &ior_write, &threefs_iov, false, fd_, current_offset, chunk_size,
...
tl::expected<size_t, ErrorCode> ThreeFSFile::vector_write(const iovec* iov,
    ...
    while (bytes_remaining > 0 && current_iov_index < iovcnt) {
        ...
        if (gpu_staging::IsDevicePointer(current_iov->iov_base, nullptr)) {
            ...
            auto ret = hf3fs_prep_npu_direct_io(
                &ior_write, &threefs_iov, false, fd_, current_offset, avail_in_iov,
                &segment_infos, reinterpret_cast<void*>(hbm_buf),
...
tl::expected<size_t, ErrorCode> ThreeFSFile::vector_read(const iovec* iov,
    ...
    while (bytes_remaining > 0 && current_iov_index < iovcnt) {
        ...
        if (gpu_staging::IsDevicePointer(current_iov->iov_base, nullptr)) {
            ...
            auto ret = hf3fs_prep_npu_direct_io(
                &ior_read, &threefs_iov, true, fd_, current_offset, avail_in_iov,
                &segment_infos, reinterpret_cast<void*>(hbm_buf),
                avail_in_iov, nullptr);
            if (ret < 0) {
                return make_error<size_t>(ErrorCode::FILE_READ_FAIL);
